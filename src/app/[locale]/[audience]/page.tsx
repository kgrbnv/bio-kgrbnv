...
const ruBusinessLinks: LinkItem[] = [
  {
    icon: Icons.calendar,
...
const enPersonalLinks: LinkItem[] = [
  {
    icon: Icons.linkedin,
    title: 'LinkedIn',
    url: 'https://www.linkedin.com/in/kgurbanov',
  ...
  },
  {
    icon: null,
    title: 'FT Board',
    subtitle: 'Boutique FinTech Consulting',
...
const enBusinessLinks: LinkItem[] = [
  {
    icon: Icons.calendar,
...
const linksMap: Record<Locale, Record<Audience, LinkItem[]>> = {
  ru: { personal: ruPersonalLinks, business: ruBusinessLinks },
  en: { personal: enPersonalLinks, business: enBusinessLinks },
}

const VALID_LOCALES: Locale[] = ['ru', 'en']
const VALID_AUDIENCES: Audience[] = ['personal', 'business']

// --- Static params ---

export function generateStaticParams() {
  return [
    { locale: 'ru', audience: 'personal' },
    { locale: 'ru', audience: 'business' },
    { locale: 'en', audience: 'personal' },
...
export function generateMetadata({ params }: { params: { locale: string; audience: string } }): Metadata {
  const isEn = params.locale === 'en'
  return {
    title: isEn ? 'Kirill Gurbanov — links' : 'Кирилл Гурбанов — ссылки',
    description: isEn
      ? 'Founder of sfer.ai. 10+ years in executive management. AI & FinTech.'
      : 'Основатель sfer.ai. 10+ лет в топ-менеджменте. Воркшопы по ИИ, телеграм-каналы и контакты.',
    openGraph: {
      title: isEn ? 'Kirill Gurbanov — links' : 'Кирилл Гурбанов — ссылки',
      description: isEn
        ? 'Founder of sfer.ai. 10+ years in executive management.'
        : 'Основатель sfer.ai. 10+ лет в топ-менеджменте.',
      url: 'https://bio.kgrbnv.com',
      siteName: isEn ? 'Kirill Gurbanov' : 'Кирилл Гурбанов',
...
export default function Page({ params }: { params: { locale: string; audience: string } }) {
  const locale = params.locale as Locale
  const audience = params.audience as Audience

  if (!VALID_LOCALES.includes(locale) || !VALID_AUDIENCES.includes(audience)) {
    notFound()
  }

  const otherLocale: Locale = locale === 'ru' ? 'en' : 'ru'
  const links = linksMap[locale][audience]

  return (
    <main className="min-h-screen flex flex-col items-center px-4 py-12 relative">
      {/* Language Switcher */}
      <Link
        href={`/${otherLocale}/${audience}`}
      ...
      <div className="w-full max-w-md">
        ...
        <div className="flex justify-center mb-6">
          <div className="avatar-border p-[3px] rounded-full">
            <div className="bg-background rounded-full p-[2px]">
              <Image
                src="/avatar.png"
                alt={locale === 'en' ? 'Kirill Gurbanov' : 'Кирилл Гурбанов'}
                width={100}
                height={100}
          ...
          <div className="glass-card rounded-full p-1 flex gap-1">
            <Link
              ...
              className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-300 ${
                audience === 'personal' ? 'tab-active' : 'tab-inactive'
              }`}
            >
              {locale === 'ru' ? 'Частным лицам' : 'Personal'}
            </Link>
            <Link
              href={`/${locale}/business`}
              className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-300 ${
                audience === 'business' ? 'tab-active' : 'tab-inactive'
              }`}
            >
              {locale === 'ru' ? 'Бизнесу' : 'Business'}
            </Link>
          </div>
        </div>
        ...
        <div className="mt-12 text-center">
          <p className="text-zinc-600 text-xs">
            &copy; {new Date().getFullYear()} {locale === 'en' ? 'Kirill Gurbanov' : 'Кирилл Гурбанов'}
          </p>
        </div>
      </div>
    </main>
